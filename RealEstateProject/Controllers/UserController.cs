...
namespace RealEstateProject.Controllers
{
    public class UserController : Controller
    {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7132/api/Advert/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        [HttpPost]
        public async Task<IActionResult> UpdateAnnouncement(UpdateAdvertDTO updateAdvertDTO)
        {
            var username = HttpContext.Session.GetString("Username");
            ...
            StringContent content = new StringContent(JsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7132/api/Advert", content);
            if (responseMessage.IsSuccessStatusCode)
            {            
